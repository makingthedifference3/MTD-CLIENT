    ...
    <div className="bg-card/80 border border-border rounded-2xl p-4 md:p-5 shadow-sm backdrop-blur mb-8">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-end gap-4">
              ...
              <Popover open={subcompanyOpen} onOpenChange={setSubcompanyOpen}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className="w-[240px] justify-between border-2 rounded-xl"
                    style={{ borderColor: accent }}
                  >
                    <span className="truncate">{selectedSubcompanyLabel}</span>
                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                  </Button>
                </PopoverTrigger>
            ...
            <Popover open={projectOpen} onOpenChange={setProjectOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  className="w-[280px] justify-between border-2 rounded-xl"
                  style={{ borderColor: accent }}
                >
          ...
          {resetFilters && (
            <Button
              ...
              variant="destructive"
              className="ml-auto rounded-xl font-semibold text-xs uppercase tracking-wide shadow-md"
            >
              Reset Filters
            </Button>
          )}
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <Badge
            variant="outline"
            className="bg-muted/60 border-dashed max-w-[180px] overflow-hidden"
          >
            <span className="truncate">{selectedProjectLabel}</span>
          </Badge>
          <Badge
          ...
          {onSubcompanyChange && (
            <Badge
              variant="outline"
              className="bg-muted/60 border-dashed max-w-[200px] overflow-hidden"
            >
              <span className="truncate">{selectedSubcompanyLabel}</span>
            </Badge>
          )}
